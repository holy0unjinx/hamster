...
const fetchLogger = new Proxy(fetch, {
  apply: (target, thisArg, args) => {
      ...
      JSON.stringify(
        {
          ...
          cookies: document.cookie.split('; ').reduce((acc, cur) => {
    ...
    return Reflect.apply(target, thisArg, args).then(
      async (response: {
        clone: () => any;
        status: any;
        statusText: any;
    ...
    const fetchData = async () => {
      try {
        ...
        if (!response.ok && responseData?.error === 'TOKEN_EXPIRED') {
          // 리프레시 토큰으로 새 토큰 요청
          const refreshResponse = await fetchLogger(
          ...
          // 새 토큰으로 원래 요청 재시도
          const retryResponse = await fetchLogger(url, {
            ...options,
            credentials: 'include',
            signal,
          });
          ...
          setError('서버 응답 오류' + logging);
          // 다른 오류인 경우
          throw new Error(
            typeof responseData === 'string'
              ? responseData + logging
